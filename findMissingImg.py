...
numbers = [[match.group(1), match.group(2)] for item in missing if (match := re.search(pattern, item))]
output = []
for item in numbers:
    item.sort()
...
for item in output:
    ...
    if not directory.count(item + ".png"):
        if os.path.isfile("./tempMapStuff/"+item+".jpg" ):
            convert1(item + ".jpg")
            set_non_transparent_to_blue(f"./{item}.png", f"./static/BoldedMap/{item}.png", 2)
